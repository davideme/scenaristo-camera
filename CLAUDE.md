...
- The Phase 0 reference matrix is one phone and one browser: a Pixel 10 and a MacBook (ADR-0017, amending PRD section 9). There is no Samsung, no iPhone and no iPad. Do not write a checklist, test or doc line that assumes a second device, and do not describe a Pixel 10 result as Android-wide or a macOS Safari result as iOS coverage. Widening the matrix is a later phase and a superseding ADR.
