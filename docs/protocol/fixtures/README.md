...
| `state.json` | The snapshot every browser mirrors, mid-recording: the flicker-safe shutter step in use, a locked focus point, a warning, the client count, the capability pair a studio look depends on (ADR-0029), and the two mid-take silences — the mount not levelling and the light not being read (ADR-0023, ADR-0027, ADR-0028). |
