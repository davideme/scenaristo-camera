...
- Bitrate follows the device's encoder profile; the app does not set one, because CameraX 1.6.2's `Recorder` derives it and offers no supported override (ADR-0002). **Measured on the reference device: 33.4 Mbit/s at H.264 4K30** (#21, confirmed at 36.4 Mbit/s on a later take), against the ≈ 80 Mbps this section previously named as a target. The earlier figures were an expectation rather than a measurement, and the files are visibly fine at the lower rate. Setting a target is **P1** and belongs with the CameraX 1.7 revisit (ADR-0002, #27), which is also where codec enforcement lands; raising it roughly doubles file size and thermal load, and #23's headroom was measured at the current rate. (Amended 2026-09-07.)
...
- **Crash resilience:** a file truncated by a crash or dead battery should be playable up to the last second. Android MVP: the CameraX 1.6.2 muxer writes the file index ahead of the media data and updates it in place inside a fixed reserve, measured at exactly 400,008 bytes on the reference device, so a killed take is complete to within the last update. Force-kills from 1 s to 60 s recover all but 0.15–0.26 s (ADR-0002 action item 4). The take length the reserve covers is **estimated at around 20 minutes and not yet confirmed**: a 24 min 48 s take's finished index measures 476,755 bytes, already past the reserve (ADR-0002 action item 4); longer takes are unplayable after a kill, and resilience for any length is P1 (ADR-0002). iOS can use `AVAssetWriter` fragment intervals from the start. [spec-chapter-markers.md](spec-chapter-markers.md) CM-1 (fragmented write plus soft-remux finalisation, which needs an app-owned muxer) is **deferred to the CameraX 1.7 revisit** (decision 2026-09-03, Davide); until then the mechanism is the one above.
...
- **Settable from the browser:** grid frequency, white balance scenario and preset, lens, and whether exposure holds for the take (ADR-0023).
- **Reported, not settable:** shutter (including 6.3's flicker-safe step), ISO, codec, audio input and level, and the lens's focal length and aperture. Shutter and ISO are *outputs* of the ADR-0005 exposure loop, and [ADR-0007](adr/0007-control-protocol.md)'s `SettingsPatch` deliberately cannot carry them — a control that cannot be honoured is worse than a missing one. What 6.3 does offer is a *lock* on the shutter, which says "stop choosing" rather than "choose this". Focus is automatic and has no control on either surface (6.1, 6.11).
- **Orientation is not a control on any surface** (amended 2026-09-07). Portrait moved to 6.11 on 2026-09-04, so v1 is landscape only and there is nothing for an orientation control to switch between. It was listed here from a draft that predated that decision.
- **Lens selection is a zoom ratio, not a camera.** On a phone the ultrawide and telephoto sit behind one logical camera and cannot be selected; the browser offers the framings the device reports, each labelled by its 35 mm-equivalent focal length. (Amended 2026-09-07; `spec-phone-and-remote-ui.md` UI-21.)
...
- **How the room is lighting the subject** (added 2026-09-08, [ADR-0028](adr/0028-portrait-lighting-read.md), UI-24). The app reads two relationships off the frames it is already metering and reports them without acting on either: how much brighter the lit half of the face is than the shadow half, and how far the background sits below the face. Studio practice for a talking head puts the first near 2:1 and the second one to two stops down; the commonest domestic fault inverts the second, which is a window behind the speaker and is not something any camera setting fixes. It is the first measurement in the product that is of the *room* rather than of the equipment, and each limit is a decision: it reports **nothing at all when there is no face**, because the meter falls back to a fixed rectangle then and a ratio across that rectangle is a fact about a rectangle; it **stops for the duration of a take**, because the reading exists so somebody can move a lamp and a take in progress is when nobody will (ADR-0023, ADR-0027's precedent); and it **decides nothing** — no warning, no automatic change. Whether a creator is offered anything on the strength of it is a later decision, taken once the number has been watched against real setups.
- **A studio look** (added 2026-09-08, [ADR-0029](adr/0029-studio-look-at-1080p-with-ml-kit.md), UI-25). Two looks, chosen once and remembered like the mains frequency: *Interview* (Rembrandt) and *Desk & beauty* (Clamshell). The app shapes the light already on the face toward the ratio that look wants and moves the background to sit under it — it redistributes and never adds, which is why it is offered only when the room has light to work with. **It costs resolution on devices that cannot bind an analysis stream beside a UHD recording**, which is measured per device rather than assumed, and the browser says so beside the choice. Off by default: the product's own answer is still that the app records what is in front of the lens.
