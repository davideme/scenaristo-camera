...
| [0033](0033-native-shutter-priority-on-android-16.md) | Use Android 16's `CONTROL_AE_PRIORITY_MODE_SENSOR_EXPOSURE_TIME_PRIORITY` on cameras that declare it; ADR-0005's metering loop becomes the rung below, and the shutter ladder and both warnings stay ours on both rungs | Proposed | 6.1, 6.2, 6.3, 8-Q3 |
...
| ADR-0033 vs ADR-0005, ADR-0013, ADR-0022, ADR-0023 | ADR-0005's Context opens with "PRD 6.3 specifies shutter-priority behaviour that neither platform offers natively", true when written and false on Android from API 36. ADR-0033 adopts the platform mode where a camera declares it, which **narrows** the others rather than leaving them intact: ADR-0005's loop and ADR-0022's two damping speeds become rung 2, ADR-0023's lock becomes `CONTROL_AE_LOCK` on rung 1 with the same user-visible meaning, and ADR-0013's guarantee that exposure behaves identically because both platforms run the same code holds for iOS and rung 2 only. The shutter ladder, both light warnings and the readouts stay shared across rungs. | **Awaiting Davide.** ADR-0033 is Proposed. ADR-0005 and ADR-0013 stay Accepted and unedited and ADR-0033 governs the scope; ADR-0022 and ADR-0023 are themselves Proposed and absorb the narrowing in their own text if this is accepted. |
